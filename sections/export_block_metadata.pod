...
This C<CREATE TABLE> statement tells SQLite to create a table named
C<blockstats> and prepares that table to hold zero or more rows of data with
...
=begin screen
    ...
    from xdg_base_dirs \
        import xdg_config_home, xdg_config_dirs, xdg_data_home
    ...
    @click.option("--numblocks",
                  default=100,
                  help="Number of blocks to export",
                  type=int)
    @click.option("--user",
                  default=None,
...
=begin screen
    ...
    with open(outfile, "w") as f:
        writer = csv.writer(f)
        want_values = \
            ["hash", "height", "time", "size", "tx", "difficulty"]
...
Your computer will process everything, and you'll end up with a C<blockstats>
table full of block statistics data!

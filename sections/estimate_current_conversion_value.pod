...
Litecoin can I get for 1 Rupee".
...
=begin screen
  ...
  $ B<curl -s \>
    B<https://api.coinranking.com/v2/coin/a91GCGd_u96cF/price | \>
    B<jq .data.price>
  "0.06701734282680077"
...
=begin screen
  ...
  $ B<< curl https://api.coinranking.com/v2/reference-currencies | >>
    B<< jq '.data.currencies[] | select(.name == "Turkish Lira").uuid' >>
  "Dk5T6J0UtCUA"
...
=begin screen
  ...
  $ B<export CURCODE="referenceCurrencyUuid=Dk5T6J0UtCUA">
  $ B<curl -s "$API/a91GCGd_u96cF/price?$CURCODE" | jq .data.price>

...
Any node that participates in the Dogecoin network has to adhere to a few
rules, mostly around behaving well, following network consensus, and
transmitting good data: basically I<behaving well>. Good nodes also help keep
the network robust, by transmitting necessary data (block history, pending
transactions) and enforcing network guidelines (block size, difficulty, fees,
et cetera).
...
How do you know? How do you do this?

=head2 *Peer Into Your Peers

X<< RPC commands; C<getpeerinfo> >>

Fortunately, your core node tracks a lot of data about the peer nodes it's
connected to, and this is available to you via the C<getpeerinfo> RPC command.
...
C<addr> shows the IP address of the peer, whether IPv4 or IPv6. C<addrlocal>
shows how the peer connects to your node. C<version> shows the version of the
protocol in use, and you want to see C<70015> for lots of reasonsN<Reading the
source code will help, so if that's not something you want to do right now,
remember to ask someone you trust about it in the future.>. C<subver> shows the
Core version in use as well as any user comment the peer added
(L<set_your_node_comment>). Finally, C<startingheight> shows the oldest block
height the peer has available.

What makes a node good? That's up to you to figure out, but one place to start
is "running the latest software" and "has a lot of blocks available".

=head2 *Filter Good Peers

X<< external programs; C<jq> >>

Let's start with the easy question. Which peers you're currently connected to
are running the latest Core version? At the time of this writing, 1.14.6 has
been out for a while, so most peers should have updated by now. The
C<getpeerinfo> command's output is a big blob of JSON with a list of objects,
so a little bit of C<jq> work will do. Assuming you've stored the output of
C<getpeerinfo> into a file named F<peers>, you might write:
...
with more blocks, you can filter on C<startingheight> instead:
...
version; storing the entire blockchain takes up some space after all.

...
You know about addresses, transactions, and blocks. You may wonder "what
happens to a transaction that isn't yet mined into a block". The answer is, as
you might expect, slightly complicated. When you create a transaction and
submit it to the network, it gets broadcast through the network until it
reaches a miner that mines it into a block.

Transmission through the network and mining all depends on the transaction
being valid: transaction inputs are valid and haven't been spent, transaction
signature is valid, transaction fees are appropriate. If enough nodes and the
miner validate all of this information, the transaction is available to be
mined into a block.
...
the mempool, adding up to 476 thousand bytes in size. This costs the Core 1.458
thousand bytes of memory, with a maximum memory size allowed of 300 million
bytes. Finally, the Core estimates no minimum fee to process the
...
Assume you've used your RPC access mechanism (L<enhance_rpc_calls>) to fetch
the current mempool size. This gives you a semi-random number you can use in
other ways.

How do you turn a number into a meme? There are plenty of options, but one easy
option at the time of this writing is C<imgflip.com>N<See the API at
U<https://imgflip.com/api>.>. If you fetch the url
U<https://api.imgflip.com/get_memes>, you get back a JSON data structure with a
list of memes (actually meme templates, but hold that thought).
...
This code is straightforwardN<Okay, if you understand Perl.>. The first couple
of lines load two libraries, C<Modern::Perl> to set language defaults and
...
C<@$memes> gets the length of the array of memes returned from the API call.

That leaves one interesting line of code, using Perl's modulus operator
...
with this if you like; hold that thought.

=head2 *What Can You Do With This?

There aren't many risks from using this code, as long as you're accessing RPC
securely (L<authenticate_rpc_securely>). Be cautious accessing any API

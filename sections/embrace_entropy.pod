...
The key to good a secret is to limit the ability of anyone to guess it. When
you're dealing with the secret keys that keep your cryptocurrency under your
control--and only your control--the better your secrets, the safer you are.
Think of it this way: if Dogecoin only allowed four-digit passwords and those
passwords had to use only the digits 0 - 9, there are only 10,000 possible passwords.

Even a novice hacker could try all 10,000 possible passwords in a short
timeN<Definitely less than weeks. Minutes, probably.>

The good thing about established cryptocurrencies such as Dogecoin is that they
use much larger numbers. In theory, there are more possible numbers than there
are atoms in the universe. Guessing any one specific number randomly will take
a novice hacker quite a whileN<That means I<never>, at least as we understand
computers now.>.
...
generation in a tool called the C<bx>, the Libbitcoin explorer. While that tool
could generate appropriate secret passphrases for Bitcoin, Litecoin, Dogecoin,
et cetera, it had a flaw that it generated far fewer possibilities than
necessary--few enough that a clever hacker could reveal multiple secrets in
hours.

The problem is a lack of entropy.
...
a truly random system, cryptographically secure, you might find a Dogecoin
private key that has the string "12345" or "CAk3" in it. That's not a bad
thing; that's your brain pulling patterns out of apparent chaos. There are
probably countless examples of these strings in public and private keys and
countless other examples where your brain might say "Hey, that looks familiar!"
That doesn't mean there's not enough entropy.
...
That's why entropy is so essential: true random input with enough of a range of
possible values that the number of things an attacker would have to search to
find anything useful is so large that it's effectively impossible.
...
This is a lot of heady math concepts. What does it mean for friendly/funny dog
money? It means everything, at least when it comes to your security. If you
didn't understand entropy and its value, reading through the security advisory
linked earlier would be difficult. Similarly, if you didn't understand that a
hashing algorithm can only do so much with what it has, you might be tempted to
use "sw0rdfish" as your passwordN<Please, never do this!> and call it a day.
...
protect your secrets, and you can do more research on the tools and techniques
available to you.

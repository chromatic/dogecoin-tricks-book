...
this writing, version 1.14.9), use the File -> Backup Wallet menu item to
...
You can achieve the same outcomes with the CLI/RPC tools. For example, use the
C<dogecoin-cli> utility to run C<backupwallet> RPC command, providing a
filename argument. The Core will back up your wallet to a file of that name
within the configured backups directory (by default,
F<.dogecoin/backups/>)N<This is the behavior as of 1.14.6.>). See
L<figure-dogecoin-cli-backup-wallet>.  Similarly, the C<encryptwallet> RPC
command can add a passphrase to a wallet.

...
ability to perform arbitrary queries on the data. You've seen some of this in
L<analyze_block_metadata>. You can ask even more complex questions and perform
more powerful manipulations on the data. The more SQL you understand, the more
options you have.

=head2 *Alias Common Columns

Queries in the previous tip reused the somewhat awkward manipulation of turning
the C<epochtime> column into a date column. Why not let SQLite do that for you?
While you I<could> reimport all of the data or create a new table or add a
column to calculate the date, you could just as easily change the way you
I<view> the data.

A view in SQL, at least as practiced by SQLite, is kind of like a query you can
write other queries against. Alternately you might think of it as a way to make
something that looks like a table but which contains aliases for other data.
Wouldn't it be nice to write C<SELECT date, txcount...> instead of all that
other complexity?
...
database with the name C<blockstats_dates>, create a view. That view should
have the columns from the C<blockstats> table as well as a new column, C<date>,
that represents the block's date.

Now to list all of the heights and hashes from blocks mined on your
goddaughter's birthday, you can write:
...
make subsequent queries easier to write without having to modify the data
you've already imported.

=head2 *Average Difficulty by Day

With that helpful scaffolding in place, it's easier to answer questions like
"What's the average mining difficulty every day in April 2023?" If you've
studied the previous tip, you probably only need the hint that SQLite has a
function called C<AVG> that will calculate the arithmetic mean of a data set.
...
Notice how much easier this query is to read by using the C<blockstats_dates>
view, especially because the C<WHERE> clause refers to the C<date> column
twice. It's totally okay to refer to a column in the C<WHERE> clause multiple
times like thisN<You can also write C<WHERE date BETWEEN '2023-04-01' AND
'2023-04-30'>, but notice that the range of dates searched includes both the
low and high values.>.

=head2 *Average Block Fill Rate

What if you want to see the average block fill rate over time? That's also a
...
single value, so that SQLite will avoid compounding errors in
mathematically-equivalent but additional calculations.
...
transaction to go through and the last 10 blocks have been 99% full, the
average is a lot less interesting.
...
Then use the SQL C<MAX()> and C<MIN()> functions to look for the highest and
lowest values for a column, respectively. These are aggregate functions, so if
you want to group them by day, you should C<GROUP> the date column:
...
It's interesting to see a block almost 130% full on May 11; this may indicate a
failure in data extraction, a failure in the calculation, or a failure in
understanding block size limits. Whatever the cause or causes, it's a good
reminder to verify things for yourself; always be willing to challenge someone
else's assertion and check your sources.

=head2 *What Can You Do With This?

You can take this in several directions: start making graphs about the health
of the network over time, plan your own use to be gentle on the network (or
maximize your own transaction throughput), perform advocacy for people running
...
you already have those skills, applying them here can help developers and node

...
much to manage, too unhygenic, too easy to get ice cream everywhere.
...
testing or a simple network). Either write a program or use C<curl> to send
these requests. Here's a simple example that tells the relay to close for 200
milliseconds, then turns on the relay, then lets it close:
...
C<PulseTime> command sets the time the relay will stay on in 100 millisecond
increments. As you might expect, relay 2 uses the command C<PulseTime2> and so
forth.  Similarly, the C<Power> command turns the relay on or off, with the
appropriate relay number and the subcommand C<ON> or C<OFF>.

Remember that the characters C<%20> represent a space encoded in a URL.

Once you've sent a C<PulseTime> command for a specific relay, the board will
remember that setting until you send another command (or manually reset the
board). Even if you toggle the board's power, relay 1 will still close for 200
milliseconds.
...
Use C<curl> again to send the C<Power1 ON> command. When your LED flashes on
and off for the right timing, you're ready to move on!  You should be able to
send both commands, see the LED light, and then turn off in the right amount of
time. From there, your program is working and you can integrate it with your
payment receiving webhooks!
...
of time thinking about how to get electricity from the wall to it (link to
tip).

...
notify script? As of Dogecoin Core 1.14.8, use the C<%i> option in your
C<walletnotify> configuration to include this height.

=head2 *Understand the Risks

In addition to every risk listed in the other tips, be aware that reindexing
the blockchain, whether directly with C<rescan> or implicitly with an address

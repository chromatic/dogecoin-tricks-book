...
restrict access to your node. Fortunately, the Dogecoin Core has a way to
ensure that only the people who are supposed to connect to a node can do so.
All you have to do is configure it securely.

=head2 *Understanding Secure Authentication

X<RPC; authentication>

To authenticate to a Core node--for example, to ask it for information or to do
things on your behalf--you need to prove your identity in two ways:
...
also needs to check that Jimbo has permission as C<milhouse>--by using
C<milhouse>'s password. If Jimbo tries C<bartrules>, C<imissmymom>, and
C<pantsme>, he'll have trouble. If he manages to guess C<thrillho>N<See
U<https://www.youtube.com/watch?v=nbbKsAZatao>.>, he's in.
...
This program generates a random password for you and a string you can add to
your configuration file. Keep those lines handy. First, grab the line starting
with C<rpcauth> and add it to your F<dogecoin.conf> file, then save the file.
...
1.14.6), so look in their repository for a file named F<rpcauth.py> for more
options when generating passwords. In particular, the Bitcoin utility uses more
entropy (see L<embrace_entropy>) when generating random passwords--giving you
more security.
...
preferred RPC mechanism. The C<curl> binary, because it has nothing to do with
Dogecoin or any other cryptocurrency, so if it works, you can be confident that
you have configured things correctly. The C<getdifficulty> RPC command is a
good test for connectivity.
...
code picks random values for you, with the password it provides and the salted
value.
...
Of course, changing your password (and salt) every now and then is healthy, as
...
same computer as where you're running your node. In fact, it's safer if you
don't.  That way your password and the salted, hashed password aren't on the
node at all, so they're not both available to attackers.

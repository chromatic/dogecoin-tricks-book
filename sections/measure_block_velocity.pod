...
"velocity". If your author earns $1 CAD selling maple-flavored dried apple
chips and buys $1 CAD worth of gasoline for a chainsaw and the chainsaw seller
buys $1 CAD worth of poutine all in the same week, that's $3 CAD worth of
velocity from the same Canadian dollar coin trading hands. How cool to watch a
loonie spread through the economy: $3 CAD worth of value created from a single
$1 CAD coin!
...
=begin screen
    ...
    sub mined($h) {
        return $h * 1_000_000 if $h < 100_000;

        return ($h - 99_999) * 500_000 + mined( 99_999 )
            if $h < 145_000;

        return ($h - 144_999) * 250_000 + mined( 144_999 )
            if $h < 200_000;

        return ($h - 199_999) * 125_000 + mined( 199_999 )
            if $h < 300_000;

        return ($h - 299_999) * 67_500 + mined( 299_999 )
            if $h < 400_000;

        return ($h - 399_999) * 31_250 + mined( 399_999 )
            if $h < 500_000;

        return ($h - 499_999) * 15_625 + mined( 499_999 )
            if $h < 600_000;

        return ($h - 599_999) * 10_000 + mined( 599_999 );
    }

    sub test_rewards {
        is mined( 1 ), 1_000_000;
        is mined( 2 ), 2_000_000;
        is mined( 99_999 ), (1_000_000 * 99_999);

        is mined(100_000), mined(99_999) + 500_000;
        is mined(145_000),
            mined(99_999) + (500_000 * 45_000) + 250_000;

        is mined(200_000), mined(199_999) + 125_000;
        is mined(299_999), mined(199_999) + 125_000 * 100_000;

        is mined(300_000), mined(299_999) + 67_500;
        is mined(399_999), mined(299_999) + 67_500 * 100_000;

        is mined(400_000), mined(399_999) + 31_250;
        is mined(499_999), mined(399_999) + 31_250 * 100_000;

        is mined(500_000), mined(499_999) + 15_625;
        is mined(599_999), mined(499_999) + 15_625 * 100_000;

        is mined(600_000), mined(599_999) + 10_000;
        is mined(699_999), mined(599_999) + 10_000 * 100_000;
    }

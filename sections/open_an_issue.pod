...
there and you won't find it. The documentation will be confusing, or missing,
or full of glaring typosN<Unlike this book, in which all typos are subtle and,
very possibly, intentional.>.

In short, you're going to notice something that could be improved.

In that case, who better to ask for the improvement than you?

=head2 *How to Open an Issue

X<GitHub>

As of this writing, official Dogecoin Core development happens on GitHub
...
Nintendo Switch?", your request isn't I<official>-official until it's written
down in some persistent fashion.
...
try to do ..." or "I think there's a missing feature, because I can't figure
out how to do ...", then you're in luck. Head over to the Dogecoin Core GitHub
Issues pageN<At U<https://github.com/dogecoin/dogecoin/issues/>.>. You'll see
...
issues tab. Now you have to make a choice. Are you going to report a bug or
request a new feature?

Depending on your choice, you'll see a textbox with a lot of text already
...
good request tells a story, and that story needs at least four parts.
...
Version? The more detail here, the easier it may be to discover useful
information. This is often more helpful for bug reports than feature requests,
but sometimes features are missing because of bugs.

=item * How you will know your problem is solved. This is different from your
big goal, because this is very specific.
...
about footprint calculations (L<calculate_your_dogecoin_footprint>). What would
that feature request look like in this framework?
...
seen at a specific block. I want to do this through RPC commands, like
C<getblockcount>.

B<What prevented me:> there's no obvious RPC method that provides this data
directly or implicitly provides this data. While I could get data for each
...
B<What context is appropriate:> I tested this on a 64-bit Linux machine with a
wallet enabled and transactions indexed (L<index_all_transactions>), so I'm
confident that the node has enough data to provide this information.

B<My problem will be solved when:> I can use a single RPC command to get the
information I like in less than ten seconds.
...
please do report a bug. While developers spend a lot of time thinking about
things, testing them, and trying to improve quality, the best feedback
available is from real users doing real things in new ways and different
contexts.
...
the features the Core already provides? (Or, in the example case, is it
I<better> done in Core, because the external implementation would be expensive,

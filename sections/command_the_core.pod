...
The first time you launched the Dogecoin CoreN<Maybe that's today, and that's
great! We all have to start somewhere, sometime!> you saw a friendly dog
graphic and some text and that's about it. There are menus and options and
buttons to send or receive transactions.
...
really ought to go back and read in more detail from the earlier chapter?"
...
your data easily accessible happen behind the scenes. You're looking at a
console where you can ask the Core to perform some of those operations and make
queries on your behalf on your own timing.
...
case return a number.> command to the Core and it responded. How did you know
what to type? How did you know what response you expected?
...
you're finished, you can close the window. In the meantime, your node will
continue running and processing transactions, so if your node is connected to
the network and i you wait a couple of minutes and type C<getblockcount> again,
you'll get a higher number.

=head2 *Understand the Risks

Several tips throughout the rest of this book will discuss RPC commands,
because they're useful in the right circumstances. While a lot of these tips
assume you'll use the Dogecoin CLI client or an external program to execute the
commands, you can use the console window to test things out on your own.

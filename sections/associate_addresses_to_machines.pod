...
drink dispenser, pachinko machine, anything coin-operated!> in the back of your
family's ice cream shop. You have a little computer network already installed
to handle the display screen showing your ice cream flavors and for the
payment/inventory system you use to sell waffle cones, scoops, sprinkles, etc.

Given that you have multiple machines, if you want to take Dogecoin in exchange
for credits in your arcade machines, you need some way of identifying that a
customer has made a payment for a specific machine. There's an easy way to do
this and a difficult way to do this.
...
to swap your Ms.Pac-Man console for your buddy's Galaga console, you can swap
the hardware and everything's good.
...
route requests to individual machines. You need to design and configure a
little bit more software in this case, but you get more flexibility.

=head2 *Tracking Addresses

If you've configured each machine with some sort of network connection (see
other tips), you might have machine names mapped to IP addresses from your DHCP
serverN<If this seems more technical than you want to manage right now, barter
with an IT-savvy friend to trade a couple of pints of ice cream for DNS
setup!>. Follow a few steps to get up and running.

=head3 *Generate a Bunch of Addresses

First, you need a bunch of addresses, maybe exported from a trusted node
(L<export_wallet_addresses>) or generated from a secure HD key. Prepare these
in a text file you can import into a database.
...
The integration work now is to route some kind of action or event to these
machines when an associated address receives an appropriate payment. How do you
associate them?
...
L<export_block_metadata> for an example)N<In a larger environment, you might
prefer MySQL or PostgreSQL to handle multi-user capabilities, but that adds
management complexity you might not be ready for yet.>. You might create a
table to map Dogecoin addresses to machine names like:
...
machine. In fact, that might be a good idea (L<rotate_machine_addresses>).

To add more addresses, all you have to do is import them or add them to this
table.

=head2 *What Can You Do With This?

With this in place, what can you do? You have enough to focus on the
interesting parts of the problem now:
...
This abstraction described here is straightforward, but it adds a lot of
flexibility and power to your system. You can deal without it if you have
smaller or simpler needs, but you need something like it at some point in your
system.
...
network, lest they have an easier time to connect to any of your machines.
...
easier (L<customize_your_qr_code>), but it reduces your anonymity. Anyone who
visits your retro gaming museum and pays for individual plays against

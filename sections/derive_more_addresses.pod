...
Whether you use a batch of Dogecoin addresses for your machine
(L<associate_addresses_to_machines>) or generate new addresses for each new
customer (L<manage_tokens>), or otherwise receive Dogecoin in transactions, you
need a steady supply of new addresses. This implies a steady supply of new
public/private key pairs.

Wallet derivation (see L<use_a_hd_wallet>) simplifies this problem by
describing a hierarchy of keys. Given a single key to start with, you can
generate an infinite number of related (derived) keys as often as you need
them. To spend transactions sent to those addresses, you need the starting key
and some information about derivation but you don't have to track all of the
public/private key pairs along the way.
...
Key derivation follows BIP-32 and BIP-44 standards. The most important piece to
understand is BIP-44's notion of derivation paths. These are descriptions of
I<how> to generate additional key pairs from an existing master key. A Dogecoin
...
the purpose of the path (C<44'>), and the Dogecoin network (C<3'>). The
following path components are up to you.

The fourth component represents a unique account number. If you have multiple
arcade locations, you could use different numbers for each location (or machine
or whatever). The fifth component is either C<0> or C<1> and indicates whether
you intend for the derived address to receive external payments. You probably
want to use C<0> to indicate this. Finally, the final component is an index
number that increases with each new address you derive.
...
transaction. Even if there's currently no unspent inputs associated with that
address, if the derived address has ever received funds, the wallet can
determine this--and know to use the I<next> unused index value when it wants to
derive further addresses.
...
simplicity, start with a path of C<m'/44'/3'/0'/0/0> and increment the index
value.

If you're associating addresses with some sort of transaction marker (such an
arcade machine, a customer invoice, or a customer account), you'll probably
want to store the derived address with the index value in a persistent store
(for example, in a database) so that you can take the appropriate action when
you see a confirmed transaction to that address.

What might code look like?
...
from hardened master private keys, at least in versions 0.1.3 and laterN<At
least, 0.1.3 will, if it includes your author's code implementing this
feature.>.
...
C<Finance::Libdogecoin> (again, at least version 0.1.3). The code can look like
this:
...
I<public> key only. In that case, any exposure of that key would compromise the
I<privacy> of your public keys and addresses but not the security of your
private keys.

In the end, it's up to you to decide how to balance security, privacy, and
convenience. Filling a pool of fresh addresses will take some ongoing
maintenance, but it lessens exposure of your keys. Generating new addresses on
demand is more convenient and needs less maintenance, but it raises the risk of
master key exposure.

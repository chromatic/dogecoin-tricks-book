...
might prefer using a language binding, such as Perl's C<Finance::Libdogecoin>
(again, at least version 0.1.3), with code as:
...
generate it. B<Be aware> that it's your responsibility to manage the security
and secrecy of your private keys.
...
=begin screen
  ...
  class Deriver {
      ...
      method derive_address ($index) {
          my $path    = "m/44'/3'/0'/0/$index";
          my $address =
             get_derived_hd_address_by_path($key, $path, 0);

...
If you want to know what's going on with your node, you will be monitoring a
file called F<debug.log> in your Dogecoin directory (see
L<understand_the_debug_log>). This file gives you information about startup and
running configuration, transaction and network activity, and warnings and
errors. Depending on how you've configured your system, this file can grow
slowly or rapidly.
...
that can grow without limits can strain your resources.
...
node. That may take a few minutes. Third, you'll have to do this again unless
you do something else. Even so, this is a fine approach if you can live with
the drawbacks.
...
log file when it starts if the file is larger than 10 MB (10 million bytes). Any time
you restart your node, the Core will perform this action.
...
On Linux and Unix-like systems, look for a program called C<logrotate>. This is
a standard program that provides all of the options described earlier. For
example, you might write a configuration file like:
...
configuration directory, something like F</etc/logrotate.d/dogecoin>. If you
lack this access, save this file in one of your own directories (F<.dogecoin/>
is fine), then enable C<logrotate> in your cron system (perhaps by running
C<crontab -e>):
...
logs available (and I<not> on the production system) might help identify the
danger and react to it more quickly.

A deleted log does you no good.

Then again, neither does an unmonitored log. As with everything else, you have
to pick and choose what you're going to learn about, what you can pay attention
to, and the level of risk you can bear in various scenarios and contexts.

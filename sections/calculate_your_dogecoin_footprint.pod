...
How many transactions has a wallet transacted? The C<getwalletinfo> RPC command
returns a field, C<txcount>, showing this value:
...
Remember, you can use any C<jq> command on this data structure you want. For
example, use the command C<jq '.[].amount'> to extract the I<amount> from every
transaction.

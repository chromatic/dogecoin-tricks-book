...
What does this code need to do? It gets launched when the Core receives a new
block and it receives as an argument the height of that block.
...
=begin programlisting
    ...
    DIFFICULTY_FORMATTED=$(numfmt --grouping ${DIFFICULTY_RAW})

    notify-send \
         -i "$HOME/.icons/dogecoin.png" \
         "Dogecoin Difficulty Update" \
         "Dogecoin difficulty is now ${DIFFICULTY_FORMATTED} at block ${BLOCK_HEIGHT}"
...
The C<BLOCK_HEIGHT> assignment gives a readable name to the one parameter this
...
argument uses a path to an icon. Here, I took the Dogecoin logo from the core
(F<src/qt/res/icons/dogecoin.png>) and put it in a directory of my own making.
The second argument is the title of the notification. The third interpolates
the nicely-formatted block difficulty and the block height into the
notification body.
...
If you want to connect to a node on another machine, set up authentication
appropriately (L<authenticate_rpc_securely>) and pass the appropriate
connection arguments to the C<dogecoin-cli> call.

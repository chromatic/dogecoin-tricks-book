...
If you're doing anything more complex with Dogecoin transactions than letting
your wallet send funds, you need to be doubly careful that your transaction
does exactly no more and no less than what you intend it to do. Consider this:
the central activity of a Dogecoin transaction is to ask nodes and miners to
verify that a piece of data from your transaction provided to a little bit of
code encoded in the input transaction produces a true value I<when executed>.

This powerful and dangerous idea gives the blockchain a lot of power, but it
also means that you're asking multiple other computers to run a little computer
program on your behalf.

Programs often have bugs, and if you're creating transactions with any kind of
logic in them, you should double- and triple-check that they do what you think
they should, lest you lock up funds so they're unspendable or allow anyone else
to spend them.

This is even more true if you use third-party software to help you craft that
transaction.

=head2 *Parts of a Script

Remember that a script has two parts. First, the script itself comes from the
input transaction. This is kind of like a lock on the funds for which you need
the key to spend. The second part is the data the spending transaction
provides. In other tips (L<host_a_treasure_hunt>), this could be a password or
phrase or other puzzle.

In the timelocking tip (L<timelock_a_vault>), the script was more complex. To
...
It's also easy to guess that I<he> verified the script himself through
alternate means, figuring that, if multiple decoding methods all give the same
results, he could trust them.

X<< RPC commands; C<decodescript> >>

Open the Dogecoin Core debug console or use C<dogecoin-cli> and use the
C<decodescript> RPC command with the raw script (C<03d075...>) as the single
argument. You should get the result:
...
reveals that what Patrick claimed is correct; to send funds which can be
unlocked by fulfilling this script, use the provided address. Finally, the
C<asm> field decodes the script itself into something more or less human
friendly: the opcodes and data used to construct the script.
...
duplicate, verify that the hash matches the hash provided in the script, and
finally verify that the key provided by the spending transaction matches the
expected key and script signature (link to tip; maybe rephrase).

All that is left is for Patrick to verify that the HASH160 of the key he
intends to use matches the provided value.

X<< RPC commands; C<getrawtransaction> >>

You don't have to use this on transactions you've written yourself; any output
of C<getrawtransaction> will work. Just be aware that, at the time of this
writing, most transactions are very simple payments to the hashes of public
keys.
...
multiple times on the testnet first (L<test_first_safely>).

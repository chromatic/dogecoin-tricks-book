...
Use either C<gem install> or C<bundler> to install the C<games_dice> and
C<tty-prompt> libraries, then run the program. You'll see a text-based menu
asking how many six-sided dice to roll:

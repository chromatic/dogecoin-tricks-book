...
Of course this only applies if you're running the Dogecoin Core software;
anything else you should inspect the code yourself (or ask someone you trust to
do so) and verify the results. For example, on a Linux machine you could run
the C<netstat> utility to list all of the programs listening for network
connections:

...
things happen but most of those things aren't visible. You can go on your way,
leaving your VM running in the cloud, laptop plugged in on your desk, or tiny
device nestled in a corner of your goat barn connected to solar panels on the
roof, and your Core node will contentedly hum along. While you're enjoying the
sunshine, your node is making and receiving connections to and from other
nodes, relaying data, validating transactions, and slowly gathering new blocks.
...
command line and, of course, output will go away when the terminal does unless
...
When the Core starts up, it prints a lot of messages to the log. Most of these
...
might see something like this snippet from a recent launch on the author's
laptop:
...
There are other lines, but they're not as generally useful, so skip them for now.
...
=begin screen
  ...
  2023-04-15 21:36:09 UpdateTip: new best=af8461i... height=4678121 version=0x00620104
      log2_work=75.419327 tx=95260772 date='2023-04-15 21:34:26'
      progress=1.000000 cache=78.0MiB(10374tx)

=end screen

This example shows three different types of information, all useful. In your
log, these three lines will all be on single lines, but for the sake of the
book, they've broken across multiple lines.
...
IPv6. This is normal; nodes come and go and sometimes backhoes make entire
networks of nodes temporarily invisible.

The third line will happen a lot, approximately once every minute. That
probably gives away what's happening. This node has received a new block with
the given hash (truncated in this example), at height 4,678,121. There are now
95,260,772 transactions in the entire blockchain, and the block's mining date
was April 15, 2023 around 9:30 pm UTC. This block is the most recent block
(otherwise C<progress> would be less than 1.0 or 100%).
...
network they support, the versions of the Core they run, and their node
comments (and whatever information you can derive from things like IP addresses
...
Other times, it's just interesting to watch for a while. After all, isn't it
more satisfying to know what's happening behind the scenes now and then?
